...
public class Kruskal {
    static void kruskal(int cost[][], int n) {
        ...
        while (ne < n - 1) {
            ...
            // Find - find the representative of a and b as u and v respectively
            while (par[u] != -1)
                u = par[u];
            while (par[v] != -1)
                v = par[v];
            // Union - Check if edge select cause cyclicity?
            // (if u==v) means reject that edge, as cycle is detected
            // if(u!=v) means no cycle, so apply union operation)
            if (u != v) {
